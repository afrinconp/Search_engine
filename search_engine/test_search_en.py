...
# Filter rows where the 'language_code' column equals 'en' and reset the index
...
"""
# Initialize vector and word count


def input_en(userPrompt):
    """
    ...
    # Iterate over tokens in user input and add to vector if they are in the Word2Vec model
    ...
    # Divide vector by word count if it is greater than 0
    if word_count > 0:
        promptVector_lemma /= word_count

    # Calculate cosine similarity between user input vector and each paragraph vector in the dataframe
    var = cosine_similarity_list(df_eng['vector'], promptVector_lemma)

    # Extract title, paragraph, and paragraph number of the top 3 most similar paragraphs
    titulo = df_eng["article_name"][var[0][1]]
    paragraph = df_eng["content"][var[0][1]]
    paragraph_number = df_eng["enumeration_in_article"][var[0][1]]

    titulo2 = df_eng["article_name"][var[1][1]]
    paragraph2 = df_eng["content"][var[1][1]]
    paragraph_number2 = df_eng["enumeration_in_article"][var[1][1]]

    titulo3 = df_eng["article_name"][var[2][1]]
    paragraph3 = df_eng["content"][var[2][1]]
    paragraph_number3 = df_eng["enumeration_in_article"][var[2][1]]
